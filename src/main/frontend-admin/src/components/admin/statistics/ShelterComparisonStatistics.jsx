...
function ShelterComparisonStatistics() {
  ...
  useEffect(() => {
    // 컴포넌트 마운트 시 초기값 0으로 설정
    setDisplayStats(shelterStats.map(s => ({
      ...s,
      animatedWalks: 0,
      animatedAdoptions: 0,
      animatedReviews: 0,
    ...
    const timer = setTimeout(() => {
      setDisplayStats(shelterStats.map(s => ({
        ...s,
        animatedWalks: Math.round((s.walks / 120) * 100),
        animatedAdoptions: Math.round((s.adoptions / 32) * 100),
        animatedReviews: Math.round((s.reviews / 14) * 100),
      })));
    }, 100); // 100ms 딜레이
  ...
  return (
    <section className="shelter-comparison-section walk-section">
        ...
        <tbody>
          {displayStats.map((s, idx) => (
            <tr key={s.name} style={{ background: idx%2===0 ? '#fff' : '#f7f7f7' }}>
              ...
              <td><ProgressBar percent={s.animatedWalks} color="#6495ED" />{s.walks}</td>
              <td><ProgressBar percent={s.animatedAdoptions} color="#6495ED" />{s.adoptions}</td>
              <td><ProgressBar percent={s.animatedReviews} color="#6495ED" />{s.reviews}</td>
              <td style={{ fontWeight: 700, color: s.rating >= 4.5 ? '#28A745' : s.rating >= 4.0 ? '#6495ED' : s.rating >= 3.5 ? '#FFC107' : s.rating >= 3.0 ? '#DC3545' : '#6c757d' }}>{s.rating}</td>
            </tr>
          ))}
        </tbody>
      </table>
