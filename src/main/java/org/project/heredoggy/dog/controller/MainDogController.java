...
public class MainDogController {
    ...
    @GetMapping("/{dogs_id}/details")
    public ResponseEntity<DogResponseDTO> getFullDetailsDog(@PathVariable("dogs_id") Long dogsId) {
        DogResponseDTO response = dogService.getFullDetailsDog(dogsId);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<List<MainDogResponseDTO>> getSimilarDogsByImage(
            @RequestParam("favoriteDogId") Long favoriteDogId,
